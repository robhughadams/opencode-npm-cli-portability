...
describe("session.list", () => {
  ...
  it.instance(
    "filters by multi-word search term using FTS",
    () =>
      Effect.gen(function* () {
        yield* withSession({ title: "postgres startup optimization notes" })
        yield* withSession({ title: "sqlite query tuning" })

        const sessions = yield* SessionNs.Service.use((session) => session.list({ search: "startup opt" }))
